...
generate_files() {
    ...
    if [ -z "$ID_TYPE" ]; then
        echo "⚠️ WARNING: No @Id field found in $ENTITY_NAME. Skipping."
        return
    fi
...
EOF
    ...
    echo "✅ Generated: ${ENTITY_NAME}Repository, ${ENTITY_NAME}Service, ${ENTITY_NAME}ServiceImpl with dynamic SQL using @Table/@Column"
}
